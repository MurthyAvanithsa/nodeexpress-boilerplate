...
describe("Validate error response for filter routes", () =>{
    let filterId = "72982";
    ...
    it("Should return error for fetching a specific filter with an invalid ID", async () => {
        filterId = "78376"; 
        const response = await request(app)
            .get(`/filter/${filterId}`)
            .set("Authorization", authToken);
    ...
    it("Should return 404 Not Found when updating filter by non-existent ID", async () => {
        filterId = "r5679";  
        const response = await request(app)
            .get(`/filter/${filterId}`)
            .set("Authorization", authToken);
    ...
    it("Should return error for deleting a filter with an invalid ID", async () => {
        filterId = "r5679"; 
        const response = await request(app)
            .get(`/filter/${filterId}`)
            .set("Authorization", authToken);
